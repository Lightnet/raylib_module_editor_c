...
void GUI_Transform3D_System(ecs_iter_t *it) {
    ...
    for (int i = 0; i < it->count; i++) {
        ...
        // CubeWire *cube = ecs_get_mut_id(it->world, gui->id, ecs_id(CubeWire));
        
        // CubeWire *cube = ecs_get_mut(it->world, gui->id,  ecs_get_id(CubeWire));
        // CubeWire *cube = ecs_get_mut(it->world, gui->id, ecs_id(CubeWire)); // nope
