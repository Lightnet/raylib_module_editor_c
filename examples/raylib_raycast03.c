...
int main(void) {
    ...
    while (!WindowShouldClose()) {
        ...
        // Fixed controls: W/S for forward/backward, A/D for left/right
        ...
        // Check for cube intersection (for hover and pick)
        for (int x = 0; x < GRID_SIZE; x++) {
            for (int z = 0; z < GRID_SIZE; z++) {
                BoundingBox box = {
        ...
        if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT) && isCubeHovered) {
            selectedCubePos = hoveredCubePos;
            isCubeSelected = true;
        }
        ...
        for (int x = 0; x < GRID_SIZE; x++) {
            for (int z = 0; z < GRID_SIZE; z++) {
                ...
                // Highlight wireframe for hovered cube
