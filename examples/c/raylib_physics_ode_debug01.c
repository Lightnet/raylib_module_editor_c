...
int main() {
    ...
    while (!WindowShouldClose()) {
        ...
        // Draw wireframe (debug mode uses green, thicker lines)
        Color wire_color = debug_mode ? GREEN : BLACK;
        DrawModelWires(cube_model, (Vector3){0, 0, 0}, 1.0f, wire_color);

        // Draw ground plane wireframe in debug mode (optional)
        if (debug_mode) {
            DrawGrid(10, 1.0f); // Visualize ground plane as a grid
        }

        // Draw contact points in debug mode (optional)
        if (debug_mode) {
            for (int i = 0; i < contact_count; ++i) {
                DrawSphereWires(contact_points[i], 0.1f, 8, 8, YELLOW);
            }
        }
        ...
        Rectangle resetButtonBounds = { 10, 30, 200, 30 };
        if (GuiButton(resetButtonBounds, "Reset Cube")) {
            resetCubePosition(cube_body);
        }

        // Draw GUI: Debug toggle button
        Rectangle debugButtonBounds = { 220, 30, 200, 30 };
        ...
        // Draw HUD
        DrawFPS(10, 10);
        DrawText("Click Buttons to Randomize/Reset Cube or Toggle Debug", 10, 70, 20, DARKGRAY);
        DrawText(TextFormat("Position: X: %.2f  Y: %.2f  Z: %.2f", pos[0], pos[1], pos[2]), 10, 100, 20, DARKGRAY);
        DrawText(TextFormat("Rotation: Yaw: %.1f  Pitch: %.1f  Roll: %.1f", yaw, pitch, roll), 10, 130, 20, DARKGRAY);
        if (debug_mode) {
            DrawText(TextFormat("Contacts: %d", contact_count), 10, 160, 20, DARKGRAY);
        }
