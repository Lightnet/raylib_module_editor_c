...
// 64x64 image size, tile size 16x16
...
void Camera3DSystem(ecs_iter_t *it) {
  ...
  for (int i = 0; i < it->count; i++) {
      ...
      DrawModelWires(*(m[i].model), (Vector3){0, 0, 0}, 1.0f, GREEN); 
  }
  DrawGrid(10, 1.0f);
...
//===============================================
    ...
    ecs_entity_t block1 = ecs_entity(world, {
      .name = "Block1"
    });
    ecs_set(world, block1, Transform3D, {
    ...
    ecs_set(world, block1, block_t, {&cubeModel});
