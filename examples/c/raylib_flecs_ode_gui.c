...
// Global user data for camera and model
typedef struct {
    Camera3D camera;
    Model model;
} UserData;
...
void Render(ecs_iter_t *it) {
    UserData *userdata = ecs_get_ctx(it->world);
    if (!userdata) return;
    ...
    BeginMode3D(userdata->camera);  // Pass camera by value, not pointer

    DrawPlane((Vector3){0, 0, 0}, (Vector2){10, 10}, GRAY);
    for (int i = 0; i < it->count; i++) {
        Model temp_model = userdata->model;
...
int main() {
    ...
    // Remove UserData component - it's not needed as a component

    // Create cube entity first to get IDs
    ...
    UserData userdata = { camera, cube_model };
    ecs_set_ctx(ecs, &userdata, NULL);
    ...
    // Cleanup
    UserData *final_userdata = ecs_get_ctx(ecs);
    if (final_userdata) {
        UnloadModel(final_userdata->model);
    }
