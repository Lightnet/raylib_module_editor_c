...
// Context struct for tracking ping counts
typedef struct {
    int pings_sent;
    int pongs_received;
    char status[256];
...
int num_clients = 0;
AppContext app_ctx = {0, 0, "No ping test yet"};
...
// Server: Initialize
void server_init(AppContext *app) {
    if (is_server) return;
    if (!ev_base) {
        ev_base = event_base_new();
        if (!ev_base) {
            strcpy(app->status, "Event base creation failed");
            return;
        }
    }
    ...
    listener = evconnlistener_new_bind(ev_base, server_accept_cb, app,
                                      LEV_OPT_CLOSE_ON_FREE | LEV_OPT_REUSEABLE, -1,
                                      (struct sockaddr *)&sin, sizeof(sin));
    if (!listener) {
        strcpy(app->status, "Listener creation failed");
        if (!is_client) {
    ...
    is_server = true;
    strcpy(app->status, "Server initialized");
}

// Client: Initialize
void client_init(AppContext *app) {
    if (is_client) return;
    if (!ev_base) {
        ev_base = event_base_new();
        if (!ev_base) {
    ...
    if (!client_bev) {
        strcpy(app->status, "Client bev creation failed");
        if (!is_server) {
            event_base_free(ev_base);
            ev_base = NULL;
        }
    ...
    if (bufferevent_socket_connect(client_bev, (struct sockaddr *)&server_addr, sizeof(server_addr)) < 0) {
        strcpy(app->status, "Client connect failed");
    ...
    is_client = true;
    strcpy(app->status, "Client initialized");
}

// Server: Send ping to all connected clients
void server_send_ping(AppContext *app) {
    if (!is_server || num_clients == 0) {
        strcpy(app->status, "No clients connected or server not initialized");
        return;
    }
    app->pings_sent++;
    strcpy(app->status, "Server: Waiting for PONG...");
    for (int i = 0; i < num_clients; i++) {
...
void client_send_ping(AppContext *app) {
    if (!is_client || !client_bev) {
        strcpy(app->status, "Client not initialized");
        return;
    }
    app->pings_sent++;
    strcpy(app->status, "Client: Waiting for PONG...");
...
int main() {
    ...
    while (!WindowShouldClose()) {
        ...
        if (GuiButton((Rectangle){100, 220, 200, 50}, "Send Ping")) {
            if (is_server) {
                server_send_ping(&app_ctx);
            } else if (is_client) {
                client_send_ping(&app_ctx);
            } else {
                strcpy(app_ctx.status, "Server or client not initialized");
            }
        }
        DrawText(app_ctx.status, 100, 300, 20, BLACK);
        DrawText(TextFormat("Pings Sent: %d", app_ctx.pings_sent), 100, 330, 20, BLACK);
        DrawText(TextFormat("Pongs Received: %d", app_ctx.pongs_received), 100, 360, 20, BLACK);
        DrawText(is_server ? "Server: Running" : "Server: Not Running", 100, 390, 20, is_server ? GREEN : RED);
        DrawText(is_client ? "Client: Connected" : "Client: Not Connected", 100, 420, 20, is_client ? GREEN : RED);
        DrawText(TextFormat("Connected Clients: %d", num_clients), 100, 450, 20, BLACK);
