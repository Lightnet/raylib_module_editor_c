...
typedef struct {
    Vector3 position;         // Local position (raymath Vector3)
    Quaternion rotation;      // Local rotation (raymath Quaternion)
    Vector3 scale;           // Local scale (raymath Vector3)
    Matrix localMatrix;       // Local transformation matrix (raymath Matrix)
    Matrix worldMatrix;       // World transformation matrix (raymath Matrix)
    bool isDirty;            // Flag to indicate if transform needs recalculation
    ecs_entity_t parent;      // Explicit parent entity reference
} Transform3D;
...
// Updated system to update transforms
void UpdateTransformHierarchySystem(ecs_iter_t *it) {
    ...
    for (int i = 0; i < it->count; i++) {
        ...
        // Translate
        localMatrix = MatrixMultiply(localMatrix, MatrixTranslate(transform->position.x, transform->position.y, transform->position.z));

        // Rotate
        Matrix rot = QuaternionToMatrix(transform->rotation);
        localMatrix = MatrixMultiply(localMatrix, rot);

        // Scale
        localMatrix = MatrixMultiply(localMatrix, MatrixScale(transform->scale.x, transform->scale.y, transform->scale.z));

        transform->localMatrix = localMatrix;

        // Check for parent transform
        ecs_entity_t parent = ecs_get_parent(it->world, entity);
        transform->parent = parent; // Update explicit parent reference
        if (parent && ecs_is_valid(it->world, parent) && ecs_has(it->world, parent, Transform3D)) {
            ...
            if (parent_transform) {
                // World matrix = local matrix * parent world matrix
                transform->worldMatrix = MatrixMultiply(transform->localMatrix, parent_transform->worldMatrix);

                // Validate world matrix to prevent garbage values
                float wx = transform->worldMatrix.m12;
                float wy = transform->worldMatrix.m13;
                float wz = transform->worldMatrix.m14;
                if (fabs(wx) > 1e6 || fabs(wy) > 1e6 || fabs(wz) > 1e6) {
                    // printf("Warning: Invalid world pos (%.2f, %.2f, %.2f) for entity %s\n",
                    //        wx, wy, wz, ecs_get_name(it->world, entity) ? ecs_get_name(it->world, entity) : "(unnamed)");
                    transform->worldMatrix = transform->localMatrix; // Fallback to local matrix
                }
            } else {
                transform->worldMatrix = transform->localMatrix;
            }
        } else {
            transform->worldMatrix = transform->localMatrix; // Root entity: world = local
        }
...
// Render begin system
void RLRenderBeginSystem(ecs_iter_t *it) {
...
// Render begin system
void RLBeginCamera3DSystem(ecs_iter_t *it) {
...
int main(void) {
    InitWindow(800, 600, "Transform Hierarchy with Flecs v4.1.1 10");
    SetTargetFPS(60);
    ...
    ecs_entity_t RLBeginRenderPhase = ecs_new_w_id(world, EcsPhase);
    ecs_entity_t RLBeginCamera3DPhase = ecs_new_w_id(world, EcsPhase);
    ecs_entity_t RLCamera3DPhase = ecs_new_w_id(world, EcsPhase);
    ecs_entity_t RLEndCamera3DPhase = ecs_new_w_id(world, EcsPhase);
    ecs_entity_t RLRenderPhase = ecs_new_w_id(world, EcsPhase);
    ...
    ecs_add_pair(world, RLBeginRenderPhase, EcsDependsOn, LogicUpdatePhase); // BeginDrawing
    ecs_add_pair(world, RLBeginCamera3DPhase, EcsDependsOn, RLBeginRenderPhase); // EcsOnUpdate, BeginMode3D
    ecs_add_pair(world, RLCamera3DPhase, EcsDependsOn, RLBeginCamera3DPhase); // 3d model only
    ecs_add_pair(world, RLEndCamera3DPhase, EcsDependsOn, RLCamera3DPhase); // End Camera and EndMode3D
    ecs_add_pair(world, RLRenderPhase, EcsDependsOn, RLEndCamera3DPhase); // 2D only
    ecs_add_pair(world, RLEndRenderPhase, EcsDependsOn, RLRenderPhase); // End render to screen
    ...
    // ONLY 2D
    ecs_system_init(world, &(ecs_system_desc_t){
      .entity = ecs_entity(world, { .name = "render2d_hud_system", .add = ecs_ids(ecs_dependson(RLRenderPhase)) }),
      .query.terms = {
          { .id = ecs_id(Transform3D), .src.id = EcsSelf },
    ...
    ecs_system_init(world, &(ecs_system_desc_t){
      .entity = ecs_entity(world, { .name = "RenderBeginSystem", 
        .add = ecs_ids(ecs_dependson(RLBeginRenderPhase)) 
      }),
      .callback = RLRenderBeginSystem
    });

    // note this has be in order of the ECS since push into array. From I guess.
    // Begin Camera 3D System
    ecs_system_init(world, &(ecs_system_desc_t){
      .entity = ecs_entity(world, {
        .name = "BeginCamera3DSystem", 
        .add = ecs_ids(ecs_dependson(RLBeginCamera3DPhase)) 
      }),
      .callback = RLBeginCamera3DSystem
    });
    ...
    ecs_system_init(world, &(ecs_system_desc_t){
        .entity = ecs_entity(world, {
          .name = "Camera3DSystem", 
          .add = ecs_ids(ecs_dependson(RLCamera3DPhase))
        }),
        .query.terms = {
            { .id = ecs_id(Transform3D), .src.id = EcsSelf },
    ...
    ecs_system_init(world, &(ecs_system_desc_t){
      .entity = ecs_entity(world, {
        ...
        .add = ecs_ids(ecs_dependson(RLEndCamera3DPhase))
      }),
