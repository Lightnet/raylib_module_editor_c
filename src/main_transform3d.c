...
void GUI_Transform3D_List_System(ecs_iter_t *it) {
    ...
    for (int i = 0; i < it->count; i++) {
        ...
        ecs_iter_t transform_it = ecs_query_iter(it->world, ecs_query_new(it->world, "Transform3D"));
        int entity_count = 0;
        printf("count %d \n", transform_it.count);

        // Count entities to allocate name buffer
        while (ecs_query_next(&transform_it)) {
            entity_count += transform_it.count;
        }

        // Allocate buffer for entity names
        ...
        ecs_iter_t transform_it2 = ecs_query_iter(it->world, ecs_query_new(it->world, "Transform3D"));
        while (ecs_query_next(&transform_it2)) {
            for (int j = 0; j < transform_it2.count; j++) {
                const char *name = ecs_get_name(it->world, transform_it2.entities[j]);
                entity_names[index] = (char *)RL_MALLOC(256 * sizeof(char));
                snprintf(entity_names[index], 256, "%s", name ? name : "(unnamed)");
                entity_ids[index] = transform_it2.entities[j];
                index++;
            }
        }

        // Define the list view rectangle (right side of the window)
        Rectangle list_rect = {200, 10, 240, 580}; // Adjusted to fit 800x600 window
        int scroll_index = 0; // For scrolling, if needed
        int new_selected_index = gui->selectedIndex;

        // Draw the list view
        GuiGroupBox(list_rect, "Entity List");
        new_selected_index = GuiListView(list_rect, (const char **)entity_names, &scroll_index, gui->selectedIndex);

        // Update TransformGUI.id if selection changed
        if (new_selected_index >= 0 && new_selected_index != gui->selectedIndex) {
            if (new_selected_index < entity_count && ecs_is_valid(it->world, entity_ids[new_selected_index])) {
                gui->id = entity_ids[new_selected_index];
                gui->selectedIndex = new_selected_index;
                printf("Selected entity: %s (ID: %llu)\n",
                       entity_names[new_selected_index],
